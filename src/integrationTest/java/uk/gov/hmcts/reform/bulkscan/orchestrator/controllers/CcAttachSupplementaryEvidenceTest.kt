...
class CcdAttachSupplementaryEvidenceTest {
    ...
    private fun RequestSpecification.postToCallback(type: String = "about-to-submit") = post("/callback/{type}", type)
    private fun RequestSpecification.setCallbackFields(init: CallbackRequest.CallbackRequestBuilder.() -> CallbackRequest.CallbackRequestBuilder)
    ...
    fun `should successfully able to call the ccd event endpoint`() {
        wireMock().register(startEvent()
            .willReturn(okJson(asJson(StartEventResponse.builder().build()))))
        given()
    ...
    private fun wireMock() = WireMock(wireMockPort)

    private fun startEvent() =
        get("/caseworkers/640/jurisdictions/BULKSCAN/case-types/Bulk_Scanned/cases/someCase/event-triggers/TBD/token")
